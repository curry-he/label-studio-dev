...
class TaskListAPI(generics.ListCreateAPIView):
    ...
    def get(self, request):
        ...
        version_pk = int_from_request(request.GET, 'version', 0)
        if version_pk:
            queryset = queryset.filter(versiontask__version_id=version_pk)
            
            split = request.GET.get('split')
            if split in ['train', 'valid', 'test']:
                queryset = queryset.filter(versiontask__subset=split)
