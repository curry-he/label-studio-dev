...
def get_next_task(
    ...
    if project.active_version:
        logger.info(f'Project {project.id} has active version {project.active_version.id}, filtering tasks for labeling.')
        version_tasks = project.active_version.tasks.filter(subset='train').values_list('task_id', flat=True)
        prepared_tasks = Task.objects.filter(id__in=version_tasks)
