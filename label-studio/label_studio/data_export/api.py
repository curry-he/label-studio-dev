...
from data_manager.utils import split_dataset
...
@method_decorator(
    ...
    decorator=swagger_auto_schema(
        tags=['Export'],
        operation_summary='Split dataset into train, validation, and test sets',
        manual_parameters=[
            openapi.Parameter(
                name='id',
            ...
            openapi.Parameter(
                ...
                description='The proportion of the dataset to allocate to the training set.',
                default=0.7,
            ),
            openapi.Parameter(
                ...
                type=openapi.TYPE_NUMBER,
                in_=openapi.IN_QUERY,
                description='The proportion of the dataset to allocate to the validation set.',
                default=0.2,
            ),
            openapi.Parameter(
                name='test_split',
                type=openapi.TYPE_NUMBER,
                in_=openapi.IN_QUERY,
                description='The proportion of the dataset to allocate to the test set.',
                default=0.1,
            ),
        ],
        responses={
            200: openapi.Response(
                ...
                schema=openapi.Schema(
                    title='Split Dataset',
                    description='Dataset split into train, validation, and test sets',
                    type=openapi.TYPE_OBJECT,
                    properties={
                        'train_set': openapi.Schema(type=openapi.TYPE_ARRAY, items=openapi.Schema(type=openapi.TYPE_OBJECT)),
                        'validation_set': openapi.Schema(type=openapi.TYPE_ARRAY, items=openapi.Schema(type=openapi.TYPE_OBJECT)),
                        'test_set': openapi.Schema(type=openapi.TYPE_ARRAY, items=openapi.Schema(type=openapi.TYPE_OBJECT)),
                    },
                ),
            )
        },
...
class DatasetSplitAPI(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        train_split = float(request.GET.get('train_split', 0.7))
        validation_split = float(request.GET.get('validation_split', 0.2))
        test_split = float(request.GET.get('test_split', 0.1))

        train_tasks, validation_tasks, test_tasks = split_dataset(
            list(tasks),
            train_split,
            validation_split,
            test_split
        ...
        train_serializer = TaskSerializer(train_tasks, many=True)
        validation_serializer = TaskSerializer(validation_tasks, many=True)
        test_serializer = TaskSerializer(test_tasks, many=True)

        return Response({
            'train_set': train_serializer.data,
            'validation_set': validation_serializer.data,
            'test_set': test_serializer.data,
        })
